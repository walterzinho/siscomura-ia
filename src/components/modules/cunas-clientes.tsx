...
import { Megaphone, Loader2, Copy, Check, Sparkles as SparklesIcon, Info } from 'lucide-react';
...
function buildPrompt(form: FormData): string {
  ...
  if (!isInfor && form.clase === 'campaña-institucional-cliente' && form.nombreCliente.trim()) {
  ...
  if (form.nombreCuña.trim()) {
    lines.push(`Nombre de la ${isInfor ? 'Pieza' : 'Cuña'}: ${form.nombreCuña.trim()}`);
  }
...
export function CunasClientesGenerator() {
  ...
  const handleTipoChange = (value: string) => {
    ...
    setForm((prev) => ({
      ...prev,
      tipo: value,
      ...(wasInfor !== willBeInfor ? { clase: '', duracion: '', tematica: '', lenguaje: '' } : {}),
    }));
  };
  ...
  const getSubmitLabel = () => {
    ...
    if (isInfor) return 'Generar Infomercial';
    return 'Generar Cuña';
  };
    ...
    <TooltipProvider delayDuration={300}>
      <div className="mx-auto w-full max-w-3xl space-y-6 p-4 sm:p-6">
        ...
        <div className="flex items-start gap-4">
          <div className={`flex size-12 shrink-0 items-center justify-center rounded-xl ${isInfor ? 'bg-purple-100 text-purple-600 dark:bg-purple-950 dark:text-purple-400' : 'bg-emerald-100 text-emerald-600 dark:bg-emerald-950 dark:text-emerald-400'}`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <Select value={form.clase} onValueChange={(v) => updateField('clase', v)}>
                <SelectTrigger id="clase" className="w-full">
                  <SelectValue placeholder={isInfor ? 'Formato del infomercial...' : 'Selecciona clase...'} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="nombreCuña" className="text-sm font-medium">
                  Nombre de la {isInfor ? 'Pieza' : 'Cuña'}
                </Label>
                <Input
                  id="nombreCuña"
                  placeholder={isInfor ? 'Ej: Infomercial Ferretería El Clavo' : 'Ej: Promo Semana Santa'}
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="objetivo" className="text-sm font-medium">
                  Objetivo de la {isInfor ? 'Pieza' : 'Cuña'} o Campaña
                </Label>
                <Textarea
                  id="objetivo"
                  placeholder={isInfor
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  id="mensajeResaltar"
                  placeholder={'Ej: Los mejores precios de la región, con garantía y crédito...'}
          ...
          {!isInfor && form.clase === 'campaña-institucional-cliente' && form.nombreCliente.trim() && (
